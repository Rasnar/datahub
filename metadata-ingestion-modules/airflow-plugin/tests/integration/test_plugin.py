...
@tenacity.retry(
    ...
    retry=tenacity.retry_if_exception_type((AssertionError, requests.exceptions.RequestException)),
)
...
def _wait_for_dag_finish(airflow_instance: AirflowInstance, dag_id: str, require_success: bool) -> None:
    print("Checking if DAG is finished")
    res = airflow_instance.session.get(f"{airflow_instance.airflow_url}/api/v1/dags/{dag_id}/dagRuns", timeout=5)
    res.raise_for_status()
...
test_cases = [
    DagTestCase("simple_dag", multiple_connections=True, platform_instance=PLATFORM_INSTANCE),
    ...
    DagTestCase("custom_operator_dag", v2_only=True, platform_instance=PLATFORM_INSTANCE),
...
def test_airflow_plugin(
    ...
    with _run_airflow(
        ...
        _wait_for_dag_finish(airflow_instance, dag_id, require_success=test_case.success)
...
def _sanitize_output_file(output_path: pathlib.Path) -> None:
    ...
    def _sanitize(obj: Any) -> None:
        if isinstance(obj, dict) and "customProperties" in obj:
            replacement_props = props_process if "run_id" in obj["customProperties"] else props_job
            obj["customProperties"] = {k: replacement_props.get(k, v) for k, v in obj["customProperties"].items()}
